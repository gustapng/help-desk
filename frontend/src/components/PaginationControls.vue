...
defineProps<{ paginationInfo: Pagination<any> | null }>();
...
<template>
  <div v-if="paginationInfo && paginationInfo.total > 0" class="flex items-center justify-between border-t border-white/10 py-3">
      ...
      <div>
        <nav aria-label="Pagination" class="isolate inline-flex -space-x-px rounded-md">
          <button
            ...
            :key="link.label"
            @click="onPageChange(link)" :disabled="!link.url || link.active"
            :class="[
