  ...
  try {
    ...
    if (err.response) {
      if (err.response.status === 422) {
        ...
        message.value = err.response.data.message || 'Erro ao realizar logout.'
      } else {
              ...
              <div class="py-1">
                <MenuItem v-slot="{ active }">
                  <a
                    href="#"
                    :class="[
                      active ? 'text-gray-800 bg-gray-300 outline-hidden' : 'text-gray-500',
                ...
                <form @submit.prevent="handleSubmit">
                  <MenuItem v-slot="{ active }">
                    <button
                      ...
                      :class="[
                        active ? 'text-gray-800 bg-gray-300 outline-hidden' : 'text-gray-500',
                        'flex items-center gap-2 p-2 text-sm w-full text-left',
                      ]"
                    >
                      <LogOut></LogOut>
                      <span>Sair</span>
